...
export function getRouter() {
    ...
    router.post(
        ...
        (_req: express.Request, res: express.Response) => {
            ...
            deleteScore(playerName, (count: number) => {
    ...
    router.post(
        "/slack/delete/playername",
        (_req: express.Request, res: express.Response) => {
            let { playerName, password } = JSON.parse(
                JSON.parse(_req.body.payload).actions[0].value
            );
            if (password != expectedPassword) {
                res.status(400).send("Unallowed Operation");
                return;
