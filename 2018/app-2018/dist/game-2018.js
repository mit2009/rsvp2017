...
var GameState;
(function (GameState) {
    GameState[GameState["CANVAS_GAME_PLAYING"] = 0] = "CANVAS_GAME_PLAYING";
    GameState[GameState["CHARACTER_SELECTION"] = 1] = "CHARACTER_SELECTION";
    GameState[GameState["GAME_MENU"] = 2] = "GAME_MENU";
    GameState[GameState["HIGHSCORES"] = 3] = "HIGHSCORES";
})(GameState || (GameState = {}));
...
var longholdTimer;
var ctx;
var imageUrl = "./images/game/";
var canvasSize = 600;
var spriteProperties;
var dangerArrayProperties;
var dangerArray;
var dangleCharacter;
var keyUpIsDown;
var startingLivesNumber = 2;
var lives;
var smootsRun = 0;
var requiredLoadables = 5 + 8 + 5 + 2; // 8 is knives, 5 is dangles, 1 is bent, 1 is dead, 1 is start
var trackedLoadables = 0;
...
function setup() {
    ...
    var scene = ['', 'forest_', 'mit_', 'space_'][Math.floor(Math.random() * 4)];
    var background_0 = scene + 'background_0';
    var background_1 = scene + 'background_1';
    var background_2 = scene + 'background_2';
    ...
    var _loop_1 = function (key) {
        var sprite = spriteProperties[key];
        var imageFileName = void 0;
        if (sprite.isCharacter) {
            var spriteColor = ['red', 'orange', 'yellow', 'green', 'blue', 'purple', 'pink', 'silver'][Math.floor(Math.random() * 8)];
        ...
        if (!sprite.totalFrames) {
            ...
            for (var frame = 0; frame < sprite.totalFrames; frame++) {
                _loop_3(frame);
            }
            if (sprite.isCharacter) {
    ...
    for (var key in spriteProperties) {
        _loop_1(key);
    }
...
function renderFrame() {
    ...
    var firstDangerElement = Math.max(0, Math.floor(-dangerArrayProperties.position.x / dangerArrayProperties.width) - 1);
    var lastDangerElement = firstDangerElement + canvasSize / dangerArrayProperties.width + 2;
    for (var dangerIndex = firstDangerElement; dangerIndex < lastDangerElement; dangerIndex++) {
        for (var knifeLevel = 0; knifeLevel < 5; knifeLevel++) {
            if (dangerArray[dangerIndex][knifeLevel] > 0 && !dangleCharacter.isDead) {
                var knifeX = dangerArrayProperties.position.x + dangerIndex * dangerArrayProperties.width;
                var knifeY = 525 - knifeLevel * (dangerArrayProperties.height + 25) - dangerArrayProperties.height - 20;
                ctx.drawImage(dangerArrayProperties.image[dangerArray[dangerIndex][knifeLevel] - 1], knifeX, knifeY);
                //check collision
                var knifeW = dangerArrayProperties.width;
                var knifeH = dangerArrayProperties.height;
                var dangleX = dangleCharacter.position.x;
                var dangleY = dangleCharacter.position.y;
                var dangleW = dangleCharacter.width;
                var dangleH = dangleCharacter.height - 10;
                if (dangleCharacter.isBent) {
                    dangleH /= 2;
                    dangleY += dangleH;
                }
                var dangleTheta = Math.atan(dangleH / (dangleW / 2));
                // check Y
                /*
                ctx.beginPath();
                ctx.moveTo(0, knifeY);
                ...
                */
                var legs = 30;
                var dangleA = (knifeY + knifeH - dangleY) / Math.tan(dangleTheta);
                var adjustedH = Math.max(dangleY + Math.tan(dangleTheta) * (knifeX - dangleX - dangleW / 2), dangleY);
...
$(function () {
    ...
    $('.jump-btn').on('mousedown touchstart', function () {
        startJump();
    });
    $('.jump-btn').on('mouseup touchend', function () {
        endJump();
    });
    $('.crouch-btn').on('mousedown touchstart', function () {
        startBent();
    });
    $('.crouch-btn').on('mouseup touchend', function () {
        endBent();
    });
    $('.retry-btn').on('mousedown touchstart', function () {
        if (dangleCharacter.isDead) {
            restart();
        }
    });
    $('.start-btn').on('mousedown touchstart', function () {
        updateLoadables();
...
/***/ 1:
/*!*************************************************************!*\
  !*** multi ./app-2018/game-scripts.ts ./app-2018/main.scss ***!
  \*************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

__webpack_require__(/*! ./app-2018/game-scripts.ts */"./app-2018/game-scripts.ts");
